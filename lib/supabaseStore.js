...
function createSupabaseStore({ url, serviceRoleKey }) {
  ...
  return {
    async getArtists({ imageSource: filter } = {}) {
      let query = supabase.from(ARTISTS_TABLE).select("*").order("name");
      if (filter) query = query.eq("photo_source", filter);
    ...
    async getAlbums({ imageSource: filter } = {}) {
      let query = supabase.from(ALBUMS_TABLE).select("*").order("title");
      if (filter) query = query.eq("cover_source", filter);
      const { data, error } = await query;
      if (error) throw error;
